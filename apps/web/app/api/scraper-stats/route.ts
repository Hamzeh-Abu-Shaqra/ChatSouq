...
export async function GET() {
  try {
    const [news, food, foodTalabat, health, shopping, services, education, hospitality, religion, people, listings] =
      await Promise.all([
        safeCount(db.execute(sql`SELECT COUNT(*) as count, MAX(scraped_at) as last_scraped FROM jordan_news`)),
        safeCount(db.execute(sql`SELECT COUNT(*) as count, MAX(scraped_at) as last_scraped FROM jordan_places WHERE category = 'food'`)),
        safeCount(db.execute(sql`SELECT COUNT(*) as count, MAX(scraped_at) as last_scraped FROM jordan_restaurants`)),
        safeCount(db.execute(sql`SELECT COUNT(*) as count, MAX(scraped_at) as last_scraped FROM jordan_places WHERE category = 'health'`)),
        safeCount(db.execute(sql`SELECT COUNT(*) as count, MAX(scraped_at) as last_scraped FROM jordan_places WHERE category = 'shopping'`)),
        safeCount(db.execute(sql`SELECT COUNT(*) as count, MAX(scraped_at) as last_scraped FROM jordan_places WHERE category = 'services'`)),
        safeCount(db.execute(sql`SELECT COUNT(*) as count, MAX(scraped_at) as last_scraped FROM jordan_places WHERE category = 'education'`)),
        safeCount(db.execute(sql`SELECT COUNT(*) as count, MAX(scraped_at) as last_scraped FROM jordan_places WHERE category = 'hospitality'`)),
        safeCount(db.execute(sql`SELECT COUNT(*) as count, MAX(scraped_at) as last_scraped FROM jordan_places WHERE category = 'religion'`)),
        safeCount(db.execute(sql`SELECT COUNT(*) as count, MAX(scraped_at) as last_scraped FROM jordan_people`)),
        safeCount(db.execute(sql`SELECT COUNT(*) as count, MAX(scraped_at) as last_scraped FROM jordan_listings`)),
      ]);
